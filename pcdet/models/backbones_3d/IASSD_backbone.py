...
class SKFusion(nn.Module):
    ...
    def __init__(self, num_channel, reduction=2):
        super().__init__()
        
#         self.fc1 = nn.Sequential(
#             nn.Conv1d(num_channel*2, num_channel//reduction, 1, bias=False),
#             nn.LeakyReLU(negative_slope=0.2),
#         )
        
#         self.fc2 = nn.Sequential(
#             nn.Conv1d(num_channel//reduction, num_channel*2, 1, bias=False),
#             nn.Sigmoid(),
#         )
        
        self.conv = nn.Sequential(
            nn.Conv1d(num_channel*2, num_channel//reduction, 1, bias=False),
            nn.BatchNorm1d(num_channel//reduction),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(num_channel//reduction, num_channel*2, 1, bias=False),
            nn.Sigmoid(),
        )
...
#         self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, agg_feats, agg_ri_feats):
        '''
        agg_feats, agg_ri_feats: [B, C, Np]
        '''
        batch_size = agg_feats.shape[0]
        
        feats = torch.cat([agg_ri_feats, agg_feats], dim=1)
        weight = self.conv(feats)
        
        return feats + feats * weight
        
#         weight = self.avg_pool(agg_feats + agg_ri_feats).reshape([batch_size, -1])
#         weight = self.fc1(weight)
#         weight = self.fc2(weight).reshape([batch_size, -1, 2])
#         weight = self.softmax(weight)
#         w1, w2 = weight[..., 0], weight[..., 1]
#         w1 = w1.reshape([batch_size, -1, 1])
#         w2 = w2.reshape([batch_size, -1, 1])
        
#         return agg_feats * w1 + agg_ri_feats * w2
...
#         self.skfusion = SKFusion(512)
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            ri_feature_input = encoder_ri_features[self.layer_inputs[i]]

            if self.layer_types[i] == 'SA_Layer':
                ctr_xyz = encoder_xyz[self.ctr_idx_list[i]] if self.ctr_idx_list[i] != -1 else None
#                 if i == 5: #comment for vanilla IA-SSD
                li_xyz, li_features, li_cls_pred, ri_feats = self.SA_modules[i](xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz, ri_features=ri_feature_input) #comment for vanilla IA-SSD
#                     ri_feats = li_features + ri_feats # [B, C=512, Np=256] #comment for vanilla IA-SSD
#                     ri_feats = torch.cat([li_features, ri_feats], dim=1) #comment for vanilla IA-SSD
#                 else: #comment for vanilla IA-SSD
#                     li_xyz, li_features, li_cls_pred = self.SA_modules[i](xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz)
            ...
            encoder_ri_features.append(ri_feats)
...
#         for i in range(len(self.SA_modules)):
#             print(encoder_features[self.layer_inputs[i]].shape)
#             print(encoder_xyz[self.layer_inputs[i]].shape)
#         print(encoder_xyz[self.layer_inputs[3]].shape)
#         print(encoder_xyz[self.layer_inputs[4]].shape)
#         print(encoder_xyz[self.layer_inputs[1]].shape)
        ...
        batch_dict['centers'] = torch.cat((ctr_batch_idx[:, None].float(), centers.contiguous().view(-1, 3)), dim=1) # [B*Np, 4] [B*256, 4]
        ...
        center_ri_features = encoder_ri_features[-1].permute(0, 2, 1).contiguous().view(-1, encoder_ri_features[-1].shape[1]) #comment for vanilla IA-SSD
#         center_mix_features = self.skfusion(encoder_ri_features[-1], encoder_features[-1]).view(-1, encoder_features[-1].shape[1]*2)
        center_mix_features = torch.cat([center_ri_features, center_features], dim=1)
        batch_dict['centers_mix_features'] = center_mix_features
        batch_dict['centers_ri_features'] = center_ri_features #comment for vanilla IA-SSD
        batch_dict['centers_features'] = center_features # [B*Np, 512] [B*256, 512]
