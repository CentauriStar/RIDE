...
#             ri_feature_input = encoder_ri_features[self.layer_inputs[i]]
            ...
            if self.layer_types[i] == 'SA_Layer':
                ...
                if i == 5: #comment for vanilla IA-SSD
                    li_xyz, li_features, li_cls_pred, ri_feats = self.SA_modules[i](xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz, ri_features=None) #comment for vanilla IA-SSD
#                     ri_feats = li_features + ri_feats # [B, C=512, Np=256] #comment for vanilla IA-SSD
                    ri_features = torch.cat([li_features, ri_feats], dim=1) #comment for vanilla IA-SSD
...
#         print(encoder_xyz[self.layer_inputs[1]].shape)
        ...
        center_ri_features = ri_feats.permute(0, 2, 1).contiguous().view(-1, ri_feats.shape[1]) #comment for vanilla IA-SSD
#         center_ri_features = encoder_ri_features[-1].permute(0, 2, 1).contiguous().view(-1, encoder_ri_features[-1].shape[1]) #comment for vanilla IA-SSD
        center_mix_features = ri_features.permute(0, 2, 1).contiguous().view(-1, ri_features.shape[1])
        ...
        batch_dict['centers_features'] = center_features # [B*Np, C=512] [B*256, 512]
