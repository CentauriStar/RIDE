...
from ...ops.Rotated_IoU.oriented_iou_loss import cal_diou_3d

class SKFusion(nn.Module):
    
    def __init__(self, num_channel, out, reduction=2):
        super().__init__()
...
#         )
        ...
        self.conv = nn.Sequential(
            nn.Linear(num_channel*2, num_channel//reduction, bias=False),
            nn.BatchNorm1d(num_channel//reduction),
            nn.ReLU(inplace=True),
            nn.Linear(num_channel//reduction, num_channel*2, bias=False),
            nn.Sigmoid()
        )
        
        self.fc = nn.Sequential(
            nn.Linear(num_channel*2, 256, bias=False),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Linear(256, 256, bias=False),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Linear(256, out, bias=True)
        )
...
#         self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, agg_ri_feats, agg_feats):
        '''
...
#         self.size_center_layers = SKFusion(detector_dim, self.box_coder.box_size)
        self.size_center_layers = self.make_fc_layers(
            fc_cfg=self.model_cfg.SIZE_FC,
            input_channels=detector_dim*2,
            output_channels=self.box_coder.box_size # 6
        )
        self.box_center_layers = self.make_fc_layers(
            fc_cfg=self.model_cfg.REG_FC,
            input_channels=detector_dim,
            output_channels=self.box_coder.code_size-self.box_coder.box_size
        )
...
#             centerness_mask = self.forward_ret_dict['iou_mask'].clone().detach()
#             one_hot_targets = one_hot_targets * centerness_mask.unsqueeze(-1).repeat(1, one_hot_targets.shape[1])
...
#         return iou3d_loss_mask, iou3d_mask
    ...
    def get_center_box_binori_layer_loss(self, tb_dict=None):
        ...
        point_box_size_preds = self.forward_ret_dict['center_size_preds']

        reg_weights = pos_mask.float()
        pos_normalizer = pos_mask.sum().float()
        reg_weights /= torch.clamp(pos_normalizer, min=1.0)

        pred_box_xyzwhl = point_box_size_preds
#         pred_box_xyzwhl = point_box_preds[:, :6]
        ...
        pred_ori_bin_id = point_box_preds[:, :self.box_coder.bin_size]
        pred_ori_bin_res = point_box_preds[:, self.box_coder.bin_size:]
...
#         pred_ori_bin_res = point_box_preds[:, 6+self.box_coder.bin_size:]
    ...
    def forward(self, batch_dict):
        ...
        center_ri_features = batch_dict['centers_ri_features']
        center_mix_features = batch_dict['centers_mix_features']
        center_coords = batch_dict['centers']
        center_cls_preds = self.cls_center_layers(center_ri_features)  # (total_centers, num_class)
#         center_size_preds = self.size_center_layers(center_ri_features, center_features)
        center_size_preds = self.size_center_layers(center_mix_features) # (B*Np, box_size=6)
        ...
        ret_dict = {'center_cls_preds': center_cls_preds,
                    'center_size_preds': center_size_preds,
        ...
        if not self.training or self.predict_boxes_when_training or \
                ...
                self.model_cfg.LOSS_CONFIG.IOU_REGULARIZATION or \
                self.model_cfg.LOSS_CONFIG.IOU3D_REGULARIZATION:
#             print(center_size_preds.shape)
#             print(center_cls_preds.shape)

            point_cls_preds, point_box_preds = self.generate_predicted_boxes(
                    points=center_coords[:, 1:4],
                    point_cls_preds=center_cls_preds, 
                    point_box_preds=torch.cat([center_size_preds,center_box_preds],dim=-1)
